...
def main(yolo):
    ...
    while True:
        ...
        for track, det in zip(tracker.tracks, detections):
            ...
            cv2.rectangle(frame, (int(trk_bbox[0]), int(trk_bbox[1])), (int(trk_bbox[2]), int(trk_bbox[3])), (255, 255, 255), 2)
            cv2.rectangle(frame, (int(det_bbox[0]), int(det_bbox[1])), (int(det_bbox[2]), int(det_bbox[3])), (255, 0, 0), 2)
            cv2.putText(frame, str(track.track_id), (int(trk_bbox[0]), int(trk_bbox[1])), 0, 5e-3 * 200, (0, 255, 0), 2)
